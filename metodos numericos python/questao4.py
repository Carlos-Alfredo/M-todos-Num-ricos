M=100
tabela = [[-3,-2,-3,0,-M,-M,0,0,0],[1,4,1,0,1,0,-1,0,7],[2,1,0,1,0,1,0,-1,10]]
colunas = ['X1','X2','X3','X4','R1','R2','S1','S2','solução']
linhas = ['Z','S1','S2']
n_colunas=9
n_linhas=3
...
maior_valor=0
for coluna in range(0,n_colunas):
	if(tabela[0][coluna]>maior_valor):
		maior_valor=tabela[0][coluna]
		maior_posicao=coluna
#O menor numero negativo na linha 0 esta na coluna menor_posicao, caso nao haja um numero negativo a posicao é -1
solucao_dividida=[]
loop_counter=0
while(maior_posicao!=-1 and loop_counter<1000): #O loop terminara quando nao houver numero negativo na linha Z
	...
	for linha in range (1,n_linhas):
		if(tabela[linha][maior_posicao]==0):
			solucao_dividida.append(float("inf"))
		else:
			solucao_dividida.append(tabela[linha][n_colunas-1]/tabela[linha][maior_posicao])
	...
	pivo=tabela[menor_solucao_posicao][maior_posicao]
	for coluna in range(0,n_colunas):
		if(pivo==0):
	...
	linhas[menor_solucao_posicao]=colunas[maior_posicao] #mudando o cabecalho da tabela
	...
	for linha in range (0,n_linhas):
		if(linha!=menor_solucao_posicao):
			if(tabela[menor_solucao_posicao][maior_posicao]==0):
				razao=float("inf")
			else:
				razao=tabela[linha][maior_posicao]/tabela[menor_solucao_posicao][maior_posicao]
			for coluna in range (0,n_colunas):
